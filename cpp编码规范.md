...
- `thread_local` 变量消耗的存储空间大小跟线程个数成正比（即使大部分线程并未实际访问），这意味着在极端场景下，可能会消耗非常大的内存。
- 普通的成员变量不能定义为 `thread_local`。
- `thread_local` 可能不如某些编译器内置支持有效。
- 常见的实现中，`thread_local` 变量会被分配在栈上，如果太大的话可能造成栈溢出。
...
如果对象很大，为了避免栈溢出，可以使用动态内存分配的方式：

```cpp
std::unique_ptr<Foo>& MyThreadLocalFoo() {
  thread_local std::unique_ptr<Foo> result = std::make_unique<Foo>(ComplicatedInitialization());
  return result;
}
```
